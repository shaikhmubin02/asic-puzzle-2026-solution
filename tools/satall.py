...
sols=[]
s2=Cadical153(bootstrap_with=cnf)
while s2.solve():
    m=set(l for l in s2.get_model() if l>0)
    b=[1 if Ivar[t] in m else 0 for t in range(NCYC)]
    sols.append(b)
    s2.add_clause([-Ivar[t] if b[t] else Ivar[t] for t in range(NCYC)])
    if len(sols)>50: break
print("TOTAL SOLUTIONS:", len(sols), file=sys.stderr)
for b in sols: print(''.join(map(str,b)))
pickle.dump(sols, open('allsols.pkl','wb'))
